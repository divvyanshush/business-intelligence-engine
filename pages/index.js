...
export default function Home() {
  ...
  const exportMarkdown = () => {
    ...
    lines.push('');
    lines.push('## Risks');
    report.risks?.forEach(r => { if (typeof r === 'string') lines.push(`- ${r}`); else { lines.push(`- **${r.r}**`); if (r.m) lines.push(`  Mitigation: ${r.m}`); }});
    lines.push('');
    lines.push('## Revenue Forecast');
    report.forecastBars?.forEach(f => lines.push(`- **${f.label}:** ${f.val}`));
    lines.push('');
    lines.push('## Score');
    if (report.scores) { const vals = Object.values(report.scores); lines.push(`**Overall: ${Math.round(vals.reduce((a,b)=>a+b,0)/vals.length)} / 100**`); lines.push(''); const labels = {marketSize:'Market Size',differentiation:'Differentiation',marginQuality:'Margin Quality',capitalEfficiency:'Capital Efficiency',executionComplexity:'Execution Complexity',scalability:'Scalability'}; Object.entries(report.scores).forEach(([k,v]) => lines.push(`- **${labels[k]||k}:** ${v}/100`)); }
    lines.push('');
    lines.push('## Competitor Landscape');
    report.competitors?.forEach(c => { lines.push(`### ${c.name} — ${c.share}% market share`); lines.push(`- Weakness: ${c.weakness}`); lines.push(`- Threat: ${c.threat}`); });
    lines.push('');
    lines.push('## Buyer Personas');
    report.personas?.forEach(p => { lines.push(`### ${p.name} — ${p.tag}`); lines.push(`- Age: ${p.age}`); lines.push(`- Channels: ${Array.isArray(p.channels)?p.channels.join(', '):p.channels}`); lines.push(`- Pain: ${p.pain}`); lines.push(`- Trigger: ${p.trigger}`); });
    lines.push('');
    lines.push('## Acquisition Channels');
    report.channels?.forEach(c => { lines.push(`### ${c.name} — ${c.fit}`); lines.push(`- CAC: ${c.cac}`); if(c.note) lines.push(`- ${c.note}`); });
    lines.push('');
    lines.push('## Legal Requirements');
    report.legal?.forEach(l => lines.push(`✓ ${l}`));
    lines.push('');
    lines.push('## Exit Potential');
    if (report.exit) { lines.push(`- Yr 3 Revenue: ${report.exit.yr3Rev}`); lines.push(`- Multiple: ${report.exit.multiple}`); lines.push(`- Valuation: ${report.exit.yr3Val}`); lines.push(`- Buyers: ${report.exit.buyers}`); }
    lines.push('');
      ...
      <div className="running-body">
        ...
        <div className="agents">
          {AGENTS.map((a, i) => {
            ...
            return (
              <div key={a.name} className={`agent ${state}`}>
                <span className="dot" />
                <span className="agent-name" data-name={a.name}>{a.name}</span>
                {state === 'done' ? (
